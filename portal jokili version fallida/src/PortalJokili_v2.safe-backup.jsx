...
// Derivar estado de una deuda a partir de saldo y vencimiento (única fuente de verdad)
...
const INITIAL_USERS = [
  ...
  { id: "m2", email: "ana@jokili.com", password: "ana123", role: "member", nombre: "Ana Rodríguez", telefono: "+58 416-555-5678", cedula: "V-15.234.567", ingreso: "2020-07-01", cargo: "Tesorera", activo: true },
...
// Deudas: montoOriginal fijo histórico, saldoPendiente se reduce con abonos
const INITIAL_DEBTS = [
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(150deg, #eef2f7 0%, #f5f7fa 50%, #edf2ee 100%)", padding: "20px" }}>
      ...
      <div className="fadeUp" style={{ width: "100%", maxWidth: "400px" }}>
        <div style={{ textAlign: "center", marginBottom: "36px" }}>
          <div style={{ width: "76px", height: "76px", margin: "0 auto 16px", background: `linear-gradient(135deg, ${C.goldDim}, ${C.goldLight})`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "34px", boxShadow: `0 4px 24px ${C.goldGlow}` }}>🎭</div>
...
const MemberPortal = ({ user, abonos, debts, notifications, onLogout }) => {
  ...
  const TABS = [
    ...
    { id: "cuotas", label: "Historial de Abonos", icon: "💳" },
    { id: "deudas", label: "Deudas", icon: "⚠️", badge: myPendingDebts.length },
    ...
    <div style={{ minHeight: "100vh", background: C.bg }}>
      ...
      <div style={{ maxWidth: "780px", margin: "0 auto", padding: "28px 16px" }}>
        <div className="fadeUp" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "12px", marginBottom: "24px" }}>
          {[
            { icon: "💳", label: "Total Abonado", val: `$${totalPaid}`, color: C.greenBright },
            { icon: "⚡", label: "Deuda Pendiente", val: `$${totalOwed}`, color: totalOwed > 0 ? "#dc2626" : C.greenBright },
            { icon: "📋", label: "Abonos Realizados", val: myAbonos.length, color: C.goldLight },
          ].map(s => (
        ...
        <div key={tab} className="fadeUp">
          {tab === "perfil" && (
            <Card>
              <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "22px", marginBottom: "20px", fontWeight: "600" }}>Mi Perfil</h2>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                {[ ["Nombre completo", user.nombre], ["Correo electr?nico", user.email], ["Tel?fono", user.telefono || "-"], ["C?dula", user.cedula || "-"], ["Cargo", user.cargo], ["Miembro desde", user.ingreso] ].map(([lbl, val]) => (
                  <div key={lbl} style={{ padding: "14px 16px", background: C.surface2, borderRadius: "10px", border: `1px solid ${C.border}` }}>
                    <div style={{ fontSize: "10px", color: C.textMuted, marginBottom: "4px", textTransform: "uppercase", letterSpacing: "0.08em" }}>{lbl}</div>
                    <div style={{ fontSize: "14px", color: C.text, fontWeight: "500" }}>{val}</div>
                  </div>
                ))}
              </div>
                <div style={{ marginTop: "16px", padding: "12px 16px", background: "#ecfdf5", border: "1px solid #a7f3d0", borderRadius: "10px", display: "flex", alignItems: "center", gap: "10px" }}>
                <span>✅</span>
                <span style={{ color: C.greenBright, fontSize: "13px" }}>Miembro activo del Jokili Verein de la Colonia Tovar</span>
              </div>
            </Card>
          )}
          {tab === "cuotas" && (
            <Card>
              ...
              {myAbonos.length === 0 ? <EmptyState icon="📋" msg="Sin abonos registrados aún" /> : (
              ...
              {myPendingDebts.length === 0 ? (
                ...
                <>
                  <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "10px", padding: "14px 18px", marginBottom: "16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "#991b1b", fontSize: "14px" }}>⚠️ Total adeudado</span>
              ...
              {notifications.length === 0 ? <Card><EmptyState icon="📢" msg="Sin avisos por ahora" /></Card> : notifications.map(n => (
                <Card key={n.id}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "10px" }}>
                    <h3 style={{ color: C.gold, fontWeight: "600", fontSize: "15px" }}>📢 {n.titulo}</h3>
...
const AdminConsole = ({ allUsers, setAllUsers, abonos, setAbonos, debts, setDebts, notifications, setNotifications, onLogout }) => {
  ...
  const [form, setForm] = useState({ nombre: "", email: "", password: "", telefono: "", cedula: "", cargo: "Miembro", activo: true });
  ...
  const openAddModal = () => {
    setEditTarget(null);
    setForm({ nombre: "", email: "", password: "", telefono: "", cedula: "", cargo: "Miembro", activo: true });
    setMemberErrors({});   //   limpia errores al abrir
    setModal(true);
  };
  const openEditModal = (m) => {
    setEditTarget(m);
    setForm({ nombre: m.nombre, email: m.email, password: m.password, telefono: m.telefono || "", cedula: m.cedula || "", cargo: m.cargo, activo: m.activo });
    setMemberErrors({});   //   limpia errores al abrir
    setModal(true);
  };
  ...
  const validarMiembro = () => {
    ...
    if (!esEmailValido(form.email))       e.email    = "Ingresa un correo válido (ej: usuario@correo.com)";
    if (form.password.length < 8)         e.password = "La contraseña debe tener mínimo 8 caracteres";
    if (!form.telefono.trim())            e.telefono = "El teléfono no puede estar vacío";
  ...
  const saveMember = () => {
    ...
    setMemberErrors({});
    if (editTarget) {
      setAllUsers(prev => prev.map(u => u.id === editTarget.id ? { ...u, ...form } : u));
      fireToast("Miembro actualizado");
    } else {
      setAllUsers(prev => [...prev, { ...form, id: "m" + Date.now(), role: "member", ingreso: new Date().toISOString().split("T")[0] }]);
  ...
  const validarDeuda = () => {
    ...
    if (!deudaForm.concepto.trim())       e.concepto    = "El concepto no puede estar vacío";
    if (!esMontoValido(deudaForm.monto))  e.monto       = "Ingresa un monto válido mayor a cero";
  ...
  // Validar y registrar abono a una deuda específica
  const validarAbono = () => {
    const e = {};
    ...
    if (!esMontoValido(abonoForm.monto)) e.monto     = "Ingresa un monto válido mayor a cero";
    if (!abonoForm.fecha)                e.fecha     = "La fecha no puede estar vacía";
  ...
  const SIDEBAR_ITEMS = [
    ...
    { id: "pagos", icon: "💰", label: "Pagos y Cargos" },
    { id: "deudas", icon: "⚠️", label: "Gestión de Deudas" },
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg }}>
      <aside style={{ width: "230px", minHeight: "100vh", background: C.sidebar, borderRight: `1px solid ${C.sidebarBorder}`, display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh" }}>
        <div style={{ padding: "24px 20px 20px", borderBottom: `1px solid ${C.sidebarBorder}` }}>
          ...
          <div style={{ fontSize: "9px", color: C.goldLight, letterSpacing: "0.2em", textTransform: "uppercase", marginTop: "4px" }}>Consola de Administración</div>
        </div>
        ...
        <div style={{ padding: "16px" }}>
          <button onClick={onLogout} style={{ width: "100%", padding: "10px", borderRadius: "8px", background: "transparent", border: `1px solid ${C.sidebarBorder}`, color: C.sidebarMuted, fontSize: "12px" }}>← Cerrar sesión</button>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, padding: "30px 28px", overflow: "auto" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: C.surface2, borderBottom: `1px solid ${C.border}` }}>
                    {["Nombre", "Email", "Cédula", "Cargo", "Estado", "Acciones"].map(h => (
                    ...
                    <tr key={m.id} style={{ borderBottom: `1px solid ${C.border}40`, background: i % 2 ? `${C.surface2}50` : "transparent" }}>
                      ...
                      <td style={{ padding: "13px 16px" }}>
                        <div style={{ display: "flex", gap: "6px" }}>
                          <Btn small variant="ghost" onClick={() => openEditModal(m)}>✏️ Editar</Btn>
        ...
        {tab === "pagos" && (
          <div className="fadeUp">
            <H1 sub="Registra deudas por concepto libre y abonos a deudas específicas">Pagos y Cargos</H1>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              <Card>
                <div style={{ display: "flex", gap: "8px", marginBottom: "18px" }}>
                  {[["deuda", "⚠️ Nueva Deuda"], ["abono", "✅ Registrar Abono"]].map(([val, lbl]) => (
              ...
              <Card>
                <div style={{ fontSize: "14px", color: C.gold, fontWeight: "600", marginBottom: "16px" }}>Últimos Registros</div>
        ...
        {tab === "deudas" && (() => {
          ...
          return (
          <div className="fadeUp">
            <H1 sub="Consulta, revisa y ordena las deudas. Para registrar abonos, usa Pagos y Cargos.">Gestión de Deudas</H1>
            {debts.length === 0 ? (
              <Card><EmptyState icon="⚠️" msg="No hay deudas registradas. Usa 'Pagos y Cargos' para agregar una." /></Card>
            ) : (
              <Card style={{ padding: 0, overflow: "hidden" }}>
                <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: "13px", color: C.textDim, fontWeight: "600" }}>
                    ...
                    &nbsp;·&nbsp;
                    <span style={{ color: "#dc2626" }}>{debts.filter(d => d.estado !== "pagada").length} activa(s)</span>
                    &nbsp;·&nbsp;
                    <span style={{ color: C.greenBright }}>{debts.filter(d => d.estado === "pagada").length} pagada(s)</span>
                  </span>
                  ...
                  {notifications.length === 0 ? <EmptyState icon="📢" msg="Sin avisos publicados" /> : notifications.map(n => (
                    <div key={n.id} style={{ padding: "14px", background: C.surface2, borderRadius: "10px", border: `1px solid ${C.border}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                        <span style={{ color: C.gold, fontWeight: "600", fontSize: "13px" }}>📢 {n.titulo}</span>
                        <span style={{ fontSize: "11px", color: C.textMuted }}>{n.fecha}</span>
                      </div>
        ...
        {tab === "reportes" && (() => {
          ...
          const sortedRows = [...filteredRows].sort((a, b) => {
            ...
            else if (reportSortCol === "estado")    { va = a.owed === 0 ? "Al día" : "Con deuda"; vb = b.owed === 0 ? "Al día" : "Con deuda"; }
            else { va = ""; vb = ""; }
            const cmp = typeof va === "number" ? va - vb : va.localeCompare(vb);
      ...
      {modal && (
        <div className="fadeIn" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }} onClick={(e) => e.target === e.currentTarget && setModal(false)}>
          <div className="fadeUp" style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "20px", padding: "32px", width: "520px", maxWidth: "95vw", maxHeight: "90vh", overflowY: "auto" }}>
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "22px", marginBottom: "22px", fontWeight: "600" }}>
              {editTarget ? "✏️ Editar Miembro" : "👤 Agregar Miembro"}
            </h2>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
              ...
              <div style={{ gridColumn: "1/-1" }}>
                ...
                <input
                  value={form.nombre}
                  onChange={e => { setForm(f => ({ ...f, nombre: e.target.value })); setMemberErrors(p => ({ ...p, nombre: "" })); }}
                  placeholder="Ej: Carlos Mendoza"
                  style={inputConError(memberErrors.nombre)}
                />
                <CampoError msg={memberErrors.nombre} />
              </div>
              ...
              <div>
                ...
                <input
                  type="email"
                  value={form.email}
                  onChange={e => { setForm(f => ({ ...f, email: e.target.value })); setMemberErrors(p => ({ ...p, email: "" })); }}
                  placeholder="carlos@jokili.com"
                  style={inputConError(memberErrors.email)}
                />
                <CampoError msg={memberErrors.email} />
              </div>
              ...
              <div>
                ...
                <input
                  type="password"
                  value={form.password}
                  onChange={e => { setForm(f => ({ ...f, password: e.target.value })); setMemberErrors(p => ({ ...p, password: "" })); }}
                  placeholder="Mínimo 8 caracteres"
                  style={inputConError(memberErrors.password)}
                />
                <CampoError msg={memberErrors.password} />
              </div>
              ...
              <div>
                ...
                <input
                  value={form.telefono}
                  onChange={e => { setForm(f => ({ ...f, telefono: e.target.value })); setMemberErrors(p => ({ ...p, telefono: "" })); }}
                  placeholder="+58 412-..."
                  style={inputConError(memberErrors.telefono)}
                />
                <CampoError msg={memberErrors.telefono} />
              </div>
              ...
              <div>
                ...
                <input
                  value={form.cedula}
                  onChange={e => { setForm(f => ({ ...f, cedula: e.target.value })); setMemberErrors(p => ({ ...p, cedula: "" })); }}
                  placeholder="V-12.345.678"
                  style={inputConError(memberErrors.cedula)}
                />
                <CampoError msg={memberErrors.cedula} />
              </div>
      ...
      {/* MODAL CONFIRMACIÓN DE ACCIÓN EN DEUDA */}
      {confirmDebt && (
        <div className="fadeIn" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 250 }} onClick={e => e.target === e.currentTarget && setConfirmDebt(null)}>
          <div className="fadeUp" style={{ background: C.surface, border: `1px solid ${C.errorBorder}`, borderRadius: "18px", padding: "28px", width: "420px", maxWidth: "92vw", boxShadow: "0 30px 80px rgba(0,0,0,0.65)" }}>
            <div style={{ width: "46px", height: "46px", borderRadius: "50%", background: C.errorBg, border: `1px solid ${C.errorBorder}`, color: C.errorColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "20px", marginBottom: "16px" }}>🗑️</div>
            ...
            <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
              ...
              <Btn variant="danger" onClick={ejecutarConfirmDebt}>Sí, eliminar</Btn>
            </div>
          </div>
        </div>
      )}
